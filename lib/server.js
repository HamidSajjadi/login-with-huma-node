...
class server {
    constructor(
        ...
        // this._url = 'https://oauth.huma.ir/api';
        this._url = 'http://194.36.174.10:2929/api';
     ...
     * @param {String} token temporary code (your client have fetched from huma login client)
     * @returns {Promise<*>}
     */
    async exchangeTemporaryCode(token) {
        let options = {
            method: 'GET',
            url: this._url + '/token',
            qs: {
    ...
    async hasUserLoggedOut(phone, macAddress) {
        let options = {
            method: 'GET',
            url: this._url + '/token/check/logout',
            qs: {
                clientSecret: this._clientSecret,
                clientId: this._clientId,
                phone,
        ...
        let result = await rp(options);
        console.log(result);
        return !!(result.hasOwnProperty('userLoggedOut') && result.userLoggedOut);
    }
